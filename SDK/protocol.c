...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_IS_AS_DETECTED, DP_TYPE_BOOL},
};
...
//    /*
    ...
    mcu_dp_enum_update(DPID_CHECKING_RESULT,0); //枚举型数据上报;
    mcu_dp_enum_update(DPID_GAS_SENSOR_STATE,0); //枚举型数据上报;
    mcu_dp_value_update(DPID_GAS_SENSOR_VALUE,10); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_PREHEAT,0); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_ALARM_SWITCH,0); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_MUFFLING,0); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_IS_AS_DETECTED,1); //BOOL型数据上报;
